...
-- used to react on connect/disconnect external displays
...
pulse.notification_timeout_seconds = 2

-- This is used later as the default terminal and editor to run.
terminal = "gnome-terminal --app-id=org.awesome"
...
myawesomemenu = {
   ...
   -- { "quit", function() awful.spawn.with_shell("xfce4-session-logout") end}
   { "quit", awesome.quit }
}
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- local t = awful.tag.find_by_name(s, "1")
    -- awful.layout.set(awful.layout.suit.tile, t)

    local t = awful.tag.find_by_name(s, "2")
    t.layout = awful.layout.suit.floating

    t = awful.tag.find_by_name(s, "8")
...
globalkeys = gears.table.join(
    ...
    awful.key({"Control", "Shift"}, "Print",
      function () awful.spawn.with_shell("echo START >>~/.gnome-screenshot.log && /bin/sleep 0.2 && xfce4-screenshooter -c -r 2>>~/.gnome-screenshot.log ; echo exit code $? >>~/.gnome-screenshot.log ; echo END >>~/.gnome-screenshot.log") end,
      {description = "screenshot an area of the screen and keep it in the clipboard",
       group = "misc"}),
    awful.key({modkey, "Control"}, "l", function () awful.spawn.with_shell("gnome-screensaver-command --lock")
    end),
    -- Audio
    ...
    awful.key({ }, "XF86HomePage",  pulse.toggle_muted_mic)
)
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift"}, "Left",
      function (c)
        awful.placement.left(c.focus)
      end,
      {description = "move the window to the left", group = "client"}),
    -- move to the right border
    awful.key({ modkey, "Shift"}, "Right",
      function (c)
        awful.placement.right(c.focus)
      end,
      {description = "move the window to the right", group = "client"}),
    -- move to the bottom
    awful.key({ modkey, "Shift"}, "Down",
      function (c)
        awful.placement.bottom(c.focus)
      end,
      {description = "move the window to the bottom", group = "client"})
)
...
awful.rules.rules = {
    ...
    { rule = { instance = "hexchat" },
      properties = { screen = screen_2_or_1,
                     tag = "8" }
    },
    { rule_any = { class = {"Telegram", "Spotify"} },
      properties = { screen = screen_2_or_1,
                     tag = "9" }
    },
}
...
-- awful.spawn.with_shell("pidof compton || compton --blur-method kawase --no-fading-openclose --backend glx  --vsync opengl-mswc --daemon")
awful.spawn.with_shell("pidof lxsession || lxsession --noautostart &")
awful.spawn.with_shell("pidof nm-applet || nm-applet --replace &")
awful.spawn.with_shell("pidof dropbox || dropbox start")
awful.spawn.with_shell("pidof nextcloud || nextcloud --background")
awful.spawn.with_shell("pidof blueman-applet || blueman-applet &")
awful.spawn.with_shell("pidof xfce4-power-manager || xfce4-power-manager &")
-- enable tap-to-click https://askubuntu.com/a/1089387
awful.spawn.with_shell("~/bin/enable-touch-click.sh")
awful.spawn.with_shell("pidof gnome-screensaver || gnome-screensaver &")
awful.spawn.with_shell("pidof redshift || redshift &")
